...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    int permission = pa ^ *pte;
    if(mappages(d, (void*)i, PGSIZE, PADDR(mem), permission) < 0)
      goto bad;
...
int mprotect(void *addr, int len){
  ...
  int residual = (int)(((int) addr) % PGSIZE );
  if(residual != 0 || len < 0 || (int)addr > USERTOP || (int)addr + len*PGSIZE > USERTOP){
    return -1;
  }
  pte_t *pte;
  int cur = (int)addr;
  while(cur < (int)addr + len*PGSIZE){
    pte = walkpgdir(proc -> pgdir, (void*)cur, 0);
    if(pte == 0){
      return -1;
    }
    *pte = *pte  &(~PTE_W);
